...
- **並行 Sub-agent**：同 stage 內最多 5 個 agent 並行（`[agents] max_threads = 5`）；不同模組的 coding/qa 可並行，PS1 序列鎖已保證同模組序列
- **Agent 定義**：`.codex/agents/<name>.toml`（project-scoped），包含 `developer_instructions`、`model`、`sandbox_mode`
- **PS1 是工具**：stage scripts 負責機械工作（同步、移動目錄、序列鎖、BackToConfirm），Codex 負責 AI 決策
- **順序**：analysis → final → coding → qa；同 stage 內並行，跨 stage 序列
...
### STEP 2：處理 analysis 任務（並行，最多 5 個）
...
**並行 spawn**（批次最多 5 個）：
"Spawn up to 5 requirements-analyst agents in parallel, one per task. Pass each task's pending_prompt.txt path as input."

### STEP 3：處理 final 任務（並行，最多 5 個）
掃描 `kingsmvpsplan/analysis/*/system/.pending_final`：

**並行 spawn**（批次最多 5 個）：
"Spawn up to 5 requirements-analyst agents in parallel for final spec tasks."
...
### STEP 5：處理 coding 任務（並行，最多 5 個，同模組序列由 PS1 保證）
掃描 `kingsmvpsplan/coding/*/system/.pending_coding`：

**並行 spawn**（批次最多 5 個）：
"Spawn up to 5 senior-software-engineer agents in parallel. PS1 module serial lock already ensures different modules only."
...
### STEP 7：處理 qa 任務（並行，最多 5 個）
掃描 `kingsmvpsplan/coding/*/system/.pending_qa`：

**並行 spawn**（批次最多 5 個）：
"Spawn up to 5 qa-analyst agents in parallel, one per task."
