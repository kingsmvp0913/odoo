...
"""
Odoo 任務同步腳本
從 Odoo 獲取指派任務，建立目錄結構和 original.txt
"""
...
def main():
    if len(sys.argv) < 7:
        print("[ERROR] 參數不足。用法: python curl.py <URL> <DB> <USER> <PWD> <USER_ID> <START_DIR> [SKIP_IDS]")
        sys.exit(1)
    ...
    for task in tasks:
        ...
        if str(task_id) in SKIP_IDS:
            print(f"[SKIP] task_{task_id} 已在 pipeline 中")
            continue

        task_dir = start_path / f"task_{task_id}"
        if task_dir.exists():
