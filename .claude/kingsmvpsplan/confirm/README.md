...
1. `pending_prompt.txt` 存在 → Claude 執行初始分析，產出 `analysis.yaml` + `.analysis_done`
2. MODE_A：等待使用者在 `analysis.yaml` 填寫 `user_answer`
3. MODE_B：PS1 自動偵測到 `execution_mode: MODE_B` 即通過
4. `analysis.ps1` STEP 3a 確認答案完整 → 寫 `.answer_done` → 移至 `analysis/`

## 離開條件

所有 `user_answer` 填寫完畢（或 MODE_B），且 `.analysis_done` 存在。
